...
data <- read_rtf("../Data/raw_syrie/CA-Toronto Star-Syrie/CA-Toronto Star-Syrie-2.rtf") #16h56
name <- "CA-Toronto Star-Syrie-2"
...
df_data2 <- df_data %>%
  mutate(variable =
           ifelse(data %in% c("Insight", "CU", "Opinion", "News",
                              "Editorial", "TV", "Entertainment",
                              "Politics", "Business", "Greater Toronto"), "section",
           ...
           ifelse(grepl("^The Toronto Star$", data), "source",
...
df_data3 <- df_data2 %>%
  ...
  pivot_wider(names_from = variable, values_from = data,
              id_cols = "article_id") %>%
  mutate(author = "NA") %>%
  select(doc_id, text, author, date, source, section) %>%
  rowwise() %>%
...
########################################################################################################### ##
 ...
 test <- readRDS("../Data/clean/CA-Toronto Star-Syrie-1.rtf.rds")
#
